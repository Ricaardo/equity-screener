...
backtest          report             React              update-all
(等权/调仓/        (Markdown/JSON)     日报查看器          install-schedule
...
| `ui/streamlit_app.py` | 1191 | Streamlit 备用看板 |
...
- **前端**：`frontend/` 使用 React + Vite + TypeScript，主入口读取报告 JSON 和附录 Markdown。
...
- **测试**：最小套件覆盖基准回测、同类去重、ETF 分类、基本面评分边界。React 前端用 `npm run build` 做类型/构建检查，UI 冒烟用 `browser-use` 截图（`scripts/check_ui_screenshots.sh`）。
...
**产品化（给人和 AI 用）**
- 报告从单一 Markdown 扩展为 **每日短摘要 + 完整附录 + JSON** 三产物，并写 `latest` 固定指针。JSON 用稳定英文键，每只候选带评分拆解、`reasons` 证据链、入选理由、主要风险、买前核验和失效条件，AI 直接读 `reports/ah-screening-report-latest.json`。
- React 看板成为**只读日报查看器**：默认展示今日摘要、优先研究、ETF 工具箱、潜力情景和证据附录，不再做实时筛选，数据源从 DB 改为报告 JSON。ETF 默认按用途分组展示，完整宽表只放进展开明细；Streamlit 保留为备用入口。
