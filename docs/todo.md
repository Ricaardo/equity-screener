...
## 美股线（us-screener）补记（2026-05-31）

此前 todo（2026-05-24）写于美股线开工前，未覆盖 `src/us_screener/`。本节补记。

### 已完成（PR #1–#5 + 2026-05-31 review）
- [x] 独立美股筛选器：独立 DuckDB、零逐个 API（stooq ZIP / Sina 批量 / SEC companyfacts.zip / FinanceDatabase / FRED / Nasdaq 财报日历），CLI + MCP + Skill。
- [x] 因子层：行业内相对估值（PE/PB/PEG peer-rank）、heat+RS 动量、宏观传导 tilt、流动性、基本面。
- [x] 宏观 EXPECTATIONS 层、相对强弱、动态主题动量、短期挤空旁注、forward 估值旁注。
- [x] 评分参数收口：`scoring_us.py` 的 composite / 估值 / heat-RS / china_master / decision / risk 权重全部移入 `ah_screener.weights`（`US_*`，附 provenance），消除散落 magic number。
- [x] forward_estimates 接线：此前是孤儿模块（仅自身测试引用），现接入 pipeline（有界 top-liquid enrich）+ report（只展示 forward PE / 分析师评级，**不进评分**）。
- [x] short_interest 口径澄清：FINRA short-volume 比率仅作报告 `squeeze_watch` 旁注，**非评分因子**（噪声代理），已加注释钉死。
- [x] 热点路径向量化：`_daily_factor_symbols`（布尔掩码）、`_decision`（np.select）去 apply/iterrows，全市场日更提速；行为由端到端测试锁定。
- [x] 宏观链路稳健性：proxy-ETF/FRED/macro-scoring 任一失败都降级为 neutral macro_score=50，并在 `macro_context.errors` 暴露原因；空均值 warning 已清除。
- [x] 剩余风险收尾：`reasons_list` / `score_components` 去掉全表 `DataFrame.apply(axis=1)`，pipeline 复用一次 `macro_context`，forward PE 改为 missing-or-stale refresh（默认 14 天），远端 stale 分支已清理。
- [x] 仓库卫生：US `*-latest.*` 报告指针纳入 .gitignore（与 ah-screening latest 指针一致）；清理已并入/过期的本地和远端残留分支；README 美股历史行情数据源口径与代码对齐（stooq 在线 CSV 不可自动化，仅用可手动下载的批量 ZIP）。

### 待办 / 已知取舍
- [ ] forward PE 仍依赖 yfinance per-symbol，限流时覆盖率会下降（已优雅降级 + `forward_pe_coverage` 暴露覆盖度；默认 14 天 stale refresh）。
- [ ] 当前工作分支 `feat/us-screener` 仍未提交/未推送本轮 review 修复；远端保留 `origin/feat/us-screener` 作为当前工作分支。
