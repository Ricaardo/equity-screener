...
> 评审范围：本方案是**渐进式重构**，不是推倒重来。现有 DuckDB schema、回测、文档解析和报告产物均保留；主看板已切到 React，Streamlit 作为备用入口。
...
当前已从“研究台长表”改为“每日摘要优先”的只读产品形态。React 前端直接消费报告 JSON 和附录 Markdown，UI 不再默认渲染完整 Markdown。

### 12.1 报告产物拆分

| 产物 | 作用 |
| --- | --- |
| `ah-screening-report-YYYY-MM-DD.md` | 给人读的每日短摘要，控制在少量段落内 |
| `ah-screening-appendix-YYYY-MM-DD.md` | 原完整长报告、覆盖率、长表和偏差说明 |
| `ah-screening-report-YYYY-MM-DD.json` | 给 UI / AI 消费的稳定结构化报告 |

JSON 新增 `daily_brief`、`top_actions`、`etf_use_cases`，候选记录新增 `why_selected`、`key_risks`、`verify_before_action`、`invalid_if`，避免前端再自行解释原始分数字段。

### 12.2 tab 重组

| tab | 内容 |
| --- | --- |
| 今日摘要 | 当前结论、数据新鲜度、今日新增/大幅变化、优先候选预览 |
| 优先研究 | 股票候选卡，默认展示入选理由、主要风险、买前核验和证据链展开 |
| ETF工具箱 | 按用途分组：核心配置、主题进攻、防御与现金、跨境与T+0、商品资源 |
| 潜力情景 | 潜力候选情景卡：触发、目标、止损、RR、证伪条件 |
| 证据附录 | 候选变化表 + 完整附录 Markdown |

### 12.3 ETF 呈现原则

- ETF 是配置工具，不按股票研究表展示。
- 默认视图按用途分组，每个用途只展示双层去重后的代表工具。
- `peer_count`、`alternatives`、`caution` 保留在卡片里，完整宽表仅在“完整 ETF 明细”展开面板中出现。
- 同簇 ETF 明确提示高度相关，避免用户同时持有多只重复暴露。

### 12.4 工程

- React 看板位于 `frontend/`，Vite dev/preview 通过 `/api/report/latest` 与 `/api/report/appendix` 读取本地 `reports/`。
- 看板只读 JSON 和附录 Markdown，不直接查询 DuckDB。
- 报告生成入口保持 `generate_report()`，但同时写短摘要、附录、JSON 和 latest 指针。
- 长表不再阻塞首屏，完整可追溯性通过附录和 JSON 保留。
- Streamlit 看板保留为备用入口，不再作为主用户路径。
